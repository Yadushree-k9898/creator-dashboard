...
const Sidebar = () => {
  ...
  return (
    <aside className="sidebar fixed left-0 top-16 w-64 h-[calc(100vh-4rem)] bg-white dark:bg-gray-800 shadow-md transform -translate-x-full md:translate-x-0 transition-all duration-300 ease-in-out z-20 overflow-y-auto flex flex-col">
      <div className="flex flex-col flex-1">
        {user && (
          <div className="p-4 border-b dark:border-gray-700 flex-shrink-0 bg-gray-50 dark:bg-gray-900/50 transition-colors duration-300">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center shadow-md transition-transform duration-300 hover:scale-110">
                <User size={20} />
              </div>
              <div>
          ...
          <ul className="space-y-1 px-3">
            {navItems.map((item) => (
              <li key={item.path} className="group">
                ...
                >
                  <span
                    className={`mr-3 transition-transform duration-300 ${isActive(item.path) ? "" : "group-hover:translate-x-1"}`}
                  >
                    {item.icon}
                  </span>
                  {item.label}
